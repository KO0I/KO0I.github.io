...
**Explicit:** Use the revised romanisation in IMG_2187: `ah → a`, `oh → o`, `ee → i`, `eh → e`, `je → ye`, `oo → u`. Preserve `uh`, `ih`, and `ay`. Native glyph keys remain the case-sensitive internal alphabet already used by `dict.js` and `alpha.js`; they are not ordinary English letters.
...
**Explicit, Lesson 21 (IMG_2198):** English “I have a flower” is `yesayn lomra'yuh ra'ye`: literally, a flower exists with me.
...
**Explicit, position sheet IMG_2188:** `o-` turns the listed spatial terms into verbs: `g'nayt → og'nayt`, “around” → “to orbit.” This evidence licenses that family, not arbitrary prefixing to all nouns.
...
**Explicit, IMG_2189 and Lesson 18:** numbers are base eight. Digit words are `nhech sto hre dosa llamih ko gol lyeway`. Decimal 8 → `stonhech`; 9 → `stosto`; 10 → `stohre`. Convert the value before writing Marain numeral sequences; don't merely replace decimal digit names.
...
| --- | --- | --- | --- |
| nayla | nAla | away; opposite direction | IMG_2188 position words |
| sein | seEn | identity | IMG_2190, Lesson 7 |
| sein'gaf | seEngaf | name | IMG_2190, Lesson 7 |
| datsha | datSa | to call; name | IMG_2190, Lesson 7 |
| metwuhshay | metwuSA | how many; how much | IMG_2194, Lesson 15 |
| hang | haG | false; incorrect | IMG_2195, Lesson 16 |
| gra | gra | true; correct | IMG_2195, Lesson 16 |
| hanggra | haGgra | is it true that | IMG_2195, Lesson 16 |
| hechkwemrawi | heHkwemrawE | meatfucker; taboo for a ship reading minds without permission | IMG_2195, Lesson 16 |
| choldicheinda | HoldEHeEnda | meat; animal matter | IMG_2195, Lesson 16 |
| kra | kra | all | IMG_2197, Lesson 20 gloss |
...
| muhksih | Preserve dictionary death and explicitly record lesson to die as an unresolved noun/verb difference | IMG_2193 handwriting; glyphs support muhksih |
...
## 9. Implementation limits and verification

The phrase assistant runs entirely in the browser. It uses longest exact English phrase matching, small explicit inflection lists, case morphology, and a small clause recogniser. It performs no network inference and uses no external model. Unknown vocabulary and genuinely ambiguous lexical matches appear literally as `<??>`, with the English span listed underneath.

For unsupported grammar it returns lexical fragments, labelled **Partial match**, instead of asserting that those fragments form a correct sentence. Known long phrases win over their components. The glyph view renders placeholders in a normal text font rather than trying to translate the placeholder into Marain glyphs. User text is inserted with `textContent`, not executable HTML.

Validation command: `node --test tests/marain-phrases.test.cjs`. Static integration checks validate unique IDs, labels, script order, and escaped placeholders. A full Jekyll build and visual browser checks were not available in the execution environment (Ruby/Jekyll and a Chromium binary are absent). These remain review checks before merging.
